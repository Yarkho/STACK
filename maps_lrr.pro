FUNCTION MAPS_LRR, maps, diff, 		plot=plot, dir=dir, string_name=string_name, out_dir=out_dir,		$
					boxcar=boxcar, int_sat=int_sat, colortable=colortable, 			$
					wsize=wsize, xsize=xsize, ysize=ysize, xrange=xrange, yrange=yrange,	$
					no_nan_corr=no_nan_corr

; Produces Log Running Ratios (LRR) out of an input map array
;
; Lrr... of Omicron Persei 8
;
...
; PLOT			- (keyword)  Set only if you require plotting, not LRR calculation
;
...
;
; HISTORY
; 2018-01-17		JD		- written
;					- Renamed to MAPS_LRR
; 2018-01-18		JD		- changed name_note to string_name
;					- added int_sat to directory name
;					- added OUT_DIR (string with the image output directory)
...
;					- added NO_NAN_CORR (inspired by plotmap.pro)
; 2018-09-08		JD		- minor corrections to documentation
...
if keyword_set(dir) then begin
  if not(is_string(dir)) or (n_elements(dir) GT 1) then begin
    message, '-!- MAPS_LRR:  malformed DIR input'
    stop
  endif
endif
...
if keyword_set(dir) then begin
  ...
  if keyword_set(colortable) then begin
    if colortable GT 74 then begin
      message, '-!- MAPS_LRR:  invalid colortable'
      stop
    endif
...
id_ext_lrr	= id_extb +'LRR '+trim(mean_dt_fix*diff) +'-sec ('+trim(string(mean_dt*diff, format='(F7.3)'))+')'

;create directory for save of the figures
if keyword_set(dir) then begin
  path		= dir+'/lrr_'+trim(mean_dt_fix*diff)+'sec_' +strmid(maps[0].id,10,3) +trim(string_name) $
		  +'_sat'+trim(int_sat)+'_boxcar'+ trim(boxcar)
  file_mkdir, path
endif


; message the user about the NaN correction
if not(keyword_set(no_nan_corr)) then begin
  print, '% MAPS_LRR: The created LRR maps have missing data replaced by 0d'
endif


; LRR construction & image writing
for imap=diff, n_elements(maps.id)-1, 1 do begin

  IF not(keyword_set(plot)) THEN BEGIN
    out_maps[imap].data		= alog10(maps[imap].data  / maps[imap-diff].data)
    out_maps[imap].id_ext	= out_maps[imap].id_ext + id_ext_lrr
  
    ;overwrite Inf, -Inf and NaN values with 0d (as if there is no change)
    if not(keyword_set(no_nan_corr)) then begin
      indf	= where(FINITE(out_maps[imap].data) EQ 0)
      if (float(!version.release) GE 8.) then begin
	if (indf[0] NE -1) or (indf[0] NE !null) then out_maps[imap].data[indf] = 0d
      endif else begin
	if (indf[0] NE -1) then out_maps[imap].data[indf] = 0d
      endelse
    endif
  ENDIF
